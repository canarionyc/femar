...
#' @importFrom utils download.file unzip str
get_tabblock <- function(){
  tabblock_fst <- file.path(.census_dir, "tabblock.fst"); print(file.info(tabblock_fst))
  if(file.exists(tabblock_fst)) {
    ...
    tabblock <- lapply(tabblock.dir, function(dsn) {
      ...
      tabblock_fst <- file.path(.census_dir, basename(dsn)) %>% fs::path_ext_set("fst")
      write_fst(tabblock_dt, tabblock_fst); print(file.info(tabblock_fst))
      return(tabblock_dt)
    }) %>% rbindlist(fill=TRUE)

    (tabblock_fst.vec <- list.files(.census_dir, pattern = "tl_2023_\\d{2}_tabblock20\\.fst", full.names = TRUE))

    rm(tabblock_sf)
    tabblock_dt <- lapply(tabblock_fst.vec, .  %>% read_fst) %>% rbindlist(fill=TRUE)

    tabblock_fst <- file.path(.census_dir, "tl_2023_ZZ_tabblock20.fst"); print(file.info(tabblock_fst))
