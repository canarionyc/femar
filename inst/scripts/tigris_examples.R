...
cbsa_tx_png <- file.path(the$CENSUS_WORKDIR, format(Sys.time(),"cbsa_tx_%Y%m%d_%H%M.png")); print(file.info(cbsa_tx_png))
library(Cairo)
...
state_cbsa_gpkg <- file.path(the$CENSUS_WORKDIR, "tx_cbsa_sf.gpkg"); print(file.info(state_cbsa_gpkg))
if(file.exists(state_cbsa_gpkg)) {
  ...
  ?st_write
  # state_cbsa_shp <- file.path(the$CENSUS_WORKDIR, "tx_cbsa_sf.shp"); print(file.info(state_cbsa_shp))
