...
# coastal states ----------------------------------------------------------
?st_intersects
states.coastline.lst <- st_intersects(states_sf, coastline_sf%>%
                                        subset(NAME %in% c("Atlantic","Atlántico","Caribbean","Gulf")), sparse = TRUE)
...
states.coastline.dist.lst <- st_is_within_distance(states_sf, coastline_sf%>%
                                        ...
                                        ,dist=dist
                                        , sparse = TRUE)
lengths(states.coastline.dist.lst) > 0
