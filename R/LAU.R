...
get_lau_es <- function(){
  ...
  if(file.exists(lau_es_gpkg)) {
    lau_es <- sf::st_read(lau_es_gpkg)

  } else {
    browseURL("E:\\Datasets\\EU\\")
    lau <- sf::st_read( "E:\\Datasets\\EU\\EC\\ref-lau-2021-01m.gdb", "LAU_RG_01M_2021_3035")

    (lau_es <- lau %>% subset(lau$CNTR_CODE=="ES" ))

    # verify the area ---------------------------------------------------------

    # lau_es$area <- sf::st_area(lau_es)
    # range(lau_es$area/lau_es$Shape_Area)
    #
    # lau_es[order(lau_es$POP_2021, decreasing = TRUE)]
    # lau_es %>% subset( grepl("Valencia",lau_es$LAU_NAME))
    # lau_es$LAU_NAME%>% sort(decreasing = FALSE) %>% tail(100L)
    #
    # lau_es[grep("València",lau_es$LAU_NAME, value = FALSE),]
    # lau_es %>% subset(lau_es$GISCO_ID=="ES_46250")
    #
